...
class RenderManager:
    ...
    def render_layer(self, layer: Layer, surface: pygame.Surface, 
        ...
        try:
            ...
            # Масштабируем до размера поля
            field_w = GRID_W * CELL_SIZE
            field_h = GRID_H * CELL_SIZE
            layer_surface = pygame.transform.scale(layer_surface, (field_w, field_h))
    ...
    def build_color_image(self, layer: Layer, rms: float, config: Dict[str, Any]) -> np.ndarray:
        ...
        ys, xs = np.nonzero(layer.grid)
        
        for i, j in zip(ys, xs):
            age = int(layer.age[i, j])
            if age <= 0:
                age = 1  # Минимальный возраст для отображения
                    
            # Базовый цвет по возрасту
            t = min(1.0, age / max(1, layer.max_age))
            
            if layer.age_palette == "Fire":
                h, s, v = hue_fire_from_t(t)
            elif layer.age_palette == "Ocean":
                h, s, v = hue_ocean_from_t(t)
            elif layer.age_palette == "Neon":
                h, s, v = hue_neon_from_t(t)
            elif layer.age_palette == "Ukraine":
                h, s, v = hue_ukraine_from_t(t)
            else:
                # Fallback: if palette is not recognized, use BGYR hue with fixed saturation and value
                h = hue_bgyr_from_t(t)
                s, v = 0.85, 1.0
            ...
            if layer.rms_enabled and layer.rms_mode != "disabled":
                ...
                if layer.rms_mode == "brightness":
                    ...
                    # Смешиваем с RMS палитрой
                    if layer.rms_palette == "Fire":
                        rms_h, rms_s, rms_v = hue_fire_from_t(rms_factor)
                    elif layer.rms_palette == "Ocean":
                        rms_h, rms_s, rms_v = hue_ocean_from_t(rms_factor)
                    elif layer.rms_palette == "Neon":
                        rms_h, rms_s, rms_v = hue_neon_from_t(rms_factor)
                    elif layer.rms_palette == "Ukraine":
            ...
            if age > fade_start:
                fade_factor = min(1.0, (age - fade_start) / max(1, layer.max_age - fade_start))
