...
def show_modern_gui(devices):
    ...
    # Начало затухания
    ttk.Label(color_frame_content, text="Начало затухания:").grid(row=1, column=0, sticky="w", pady=5)
    fade_start_var = tk.IntVar(value=60)
    ttk.Scale(color_frame_content, from_=10, to=200, variable=fade_start_var).grid(row=1, column=1, sticky="ew", padx=(10,0), pady=5)
