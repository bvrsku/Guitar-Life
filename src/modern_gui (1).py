...
def show_modern_gui(devices):
    ...
    def on_ok():
        ...
        # Собираем настройки
        # Динамически формируем описание слоёв по количеству
        layers = []
        try:
            lc = int(layer_count_var.get())
        except Exception:
            lc = 3
        lc = max(1, min(5, lc))
        ...
        for i in range(lc):
            layers.append({
                "rule": "Conway",
                "age_palette": age_palette_var.get() if "age_palette_var" in globals() else "Blue->Green->Yellow->Red",
                "rms_palette": rms_palette_var.get() if "rms_palette_var" in globals() else "Blue->Green->Yellow->Red",
                "blend_mode": "Normal",
                "alpha": 200,
                "solo": False,
                "mute": False
        ...
        result = {
            'audio': {
                'device': device_var.get() if 'device_var' in globals() else None,
                'gain': gain_var.get(),
                'rms_strength': rms_strength_var.get(),
                'rms_min': rms_min_var.get(),
                'rms_max': rms_max_var.get(),
            ...
            'visual': {
                ...
                'fade_sat_drop': fade_sat_var.get(),
                'fade_val_drop': fade_val_var.get(),
                'hue_offset': hue_offset_var.get(),
