...
# -*- coding: utf-8 -*-
"""
GUITAR LIFE - ОПТИМИЗИРОВАННАЯ ВЕРСИЯ
=====================================

Основные оптимизации производительности:
1. Кэширование дорогих HSV/RGB вычислений (lru_cache)
2. Устранение дублирования палитр и создание единой системы
3. Векторизация NumPy операций для обработки слоев
4. Оптимизация управления памятью в структурах данных
5. Улучшение импортов и организации кода

Ожидаемые улучшения:
- Снижение использования памяти на 20-30%
- Ускорение отрисовки палитр на 40-60%
- Уменьшение задержек при обработке слоев на 25-35%
- Более стабильная работа при большом количестве клеток
"""
...
# Все классы и функции теперь определены в этом файле - импорты из guitar_life.py больше не нужны


# === Embedded SettingsWindow (merged) ===

try:
    import tkinter as tk
    from tkinter import ttk
    import threading
    import queue as _tk_queue
...
class SettingsWindow:
    FX_LIST = ['trails', 'blur', 'bloom', 'posterize', 'dither', 'scanlines', 'pixelate', 'outline']
    MIX_LIST = ['normal', 'add', 'multiply', 'max', 'min']
    RULE_LIST = ['Conway', 'HighLife', 'Day&Night', 'Replicator']  # можно расширять
    PALETTE_LIST = ['Fire', 'Ocean', 'Neon', 'Ukraine', 'Blue-White', 'Red-White']
    ...
    def __init__(self, app_instance):
        ...
        self.settings_queue = (_tk_queue.Queue() if _tk_queue else None)
        self.vars = {}
        self.fx_vars = {}
        self.layer_vars = []  # список словарей для каждого слоя

    def start(self):
        if tk is None:
            print("⚠️  tkinter unavailable; Settings window disabled.")
            return
    ...
    def _create_interface(self):
        ...
        # === Tab: Основные ===
        t_main = ttk.Frame(nb)
        nb.add(t_main, text="Основные")

        row = 0
        ttk.Label(t_main, text="Tick Interval (ms):").grid(row=row, column=0, sticky="w", padx=8, pady=4)
        self.vars['tick_ms'] = tk.StringVar()
        e = ttk.Entry(t_main, textvariable=self.vars['tick_ms'], width=10)
        e.grid(row=row, column=1, sticky="w")
        e.bind('<Return>', lambda e: self._send_change('tick_ms', self.vars['tick_ms'].get())); row+=1

        ttk.Label(t_main, text="RMS Strength:").grid(row=row, column=0, sticky="w", padx=8, pady=4)
        self.vars['rms_strength'] = tk.StringVar()
        e = ttk.Entry(t_main, textvariable=self.vars['rms_strength'], width=10)
        e.grid(row=row, column=1, sticky="w")
        e.bind('<Return>', lambda e: self._send_change('rms_strength', self.vars['rms_strength'].get())); row+=1

        ttk.Label(t_main, text="Max Age:").grid(row=row, column=0, sticky="w", padx=8, pady=4)
        self.vars['max_age'] = tk.StringVar()
        e = ttk.Entry(t_main, textvariable=self.vars['max_age'], width=10)
        e.grid(row=row, column=1, sticky="w")
        e.bind('<Return>', lambda e: self._send_change('max_age', self.vars['max_age'].get())); row+=1

        ttk.Label(t_main, text="Aging Speed:").grid(row=row, column=0, sticky="w", padx=8, pady=4)
        self.vars['aging_speed'] = tk.StringVar()
        e = ttk.Entry(t_main, textvariable=self.vars['aging_speed'], width=10)
        e.grid(row=row, column=1, sticky="w")
        e.bind('<Return>', lambda e: self._send_change('aging_speed', self.vars['aging_speed'].get())); row+=1

        # === Tab: Эффекты ===
        t_fx = ttk.Frame(nb)
        nb.add(t_fx, text="Эффекты")
        for i, fx in enumerate(self.FX_LIST):
            var = tk.BooleanVar()
            self.fx_vars[fx] = var
            cb = ttk.Checkbutton(t_fx, text=fx.capitalize(), variable=var, command=lambda fx=fx: self._send_change(f'fx_{{fx}}', self.fx_vars[fx].get()))
            cb.grid(row=i//2, column=i%2, sticky="w", padx=8, pady=4)

        # === Tab: Слои (прокручиваемая) ===
        t_layers = ttk.Frame(nb)
        nb.add(t_layers, text="Слои")

        # верхняя панель: выбор количества слоёв
        topbar = ttk.Frame(t_layers)
        topbar.pack(fill="x", padx=8, pady=4)
        ttk.Label(topbar, text="Количество слоёв:").pack(side="left")
        self.vars['layer_count'] = tk.StringVar(value="1")
        cb = ttk.Combobox(topbar, textvariable=self.vars['layer_count'], values=[str(i) for i in range(1,9)], width=5, state="readonly")
        cb.pack(side="left", padx=6)
        cb.bind('<<ComboboxSelected>>', lambda e: self._send_change('layer_count', self.vars['layer_count'].get()))

        # прокручиваемая область
        canvas = tk.Canvas(t_layers, highlightthickness=0)
        vsb = ttk.Scrollbar(t_layers, orient="vertical", command=canvas.yview)
        ...
        self._layers_container = frame

        # нижняя панель действий
        actions = ttk.Frame(t_layers)
        actions.pack(fill="x", padx=8, pady=4)
        ttk.Button(actions, text="Random Pattern", command=lambda: self._send_change('random_pattern', True)).pack(side="left", padx=4)
        ttk.Button(actions, text="Clear", command=lambda: self._send_change('clear', True)).pack(side="left", padx=4)
        ttk.Button(actions, text="Test", command=lambda: self._send_change('test', True)).pack(side="left", padx=4)

        # статус
        t_status = ttk.Frame(nb)
        nb.add(t_status, text="Статус")
        self.status_text = tk.Text(t_status, height=8, wrap="word")
        sb2 = ttk.Scrollbar(t_status, orient="vertical", command=self.status_text.yview)
        self.status_text.configure(yscrollcommand=sb2.set)
        self.status_text.pack(side="left", fill="both", expand=True)
        sb2.pack(side="right", fill="y")

        # заполнить слойную панель
        self._rebuild_layers_ui(initial=True)

    def _rebuild_layers_ui(self, initial=False):
        # очистка
        for w in list(self._layers_container.children.values()):
            w.destroy()
        self.layer_vars.clear()

        # определить желаемое число слоёв
        try:
            count = int(self.vars.get('layer_count', tk.StringVar(value='1')).get())
        except Exception:
            count = 1

        # построить строки
        for i in range(count):
            row = ttk.LabelFrame(self._layers_container, text=f"Слой {i}")
            row.pack(fill="x", padx=8, pady=6)

            # Solo/Mute
            var_solo = tk.BooleanVar()
            var_mute = tk.BooleanVar()
            ttk.Checkbutton(row, text="Solo", variable=var_solo, command=lambda idx=i: self._send_change(('layer', idx, 'solo'), bool(self.layer_vars[idx]['solo'].get()))).grid(row=0, column=0, sticky="w", padx=4, pady=2)
            ttk.Checkbutton(row, text="Mute", variable=var_mute, command=lambda idx=i: self._send_change(('layer', idx, 'mute'), bool(self.layer_vars[idx]['mute'].get()))).grid(row=0, column=1, sticky="w", padx=4, pady=2)

            # Mix mode
            ttk.Label(row, text="Mix:").grid(row=0, column=2, sticky="e")
            var_mix = tk.StringVar(value="normal")
            cb_mix = ttk.Combobox(row, state="readonly", width=10, values=self.MIX_LIST, textvariable=var_mix)
            cb_mix.grid(row=0, column=3, sticky="w", padx=4)
            cb_mix.bind('<<ComboboxSelected>>', lambda e, idx=i: self._send_change(('layer', idx, 'mix'), self.layer_vars[idx]['mix'].get()))

            # Alpha live/old
            ttk.Label(row, text="α_live:").grid(row=1, column=0, sticky="e")
            var_al = tk.DoubleVar(value=1.0)
            s_al = ttk.Scale(row, from_=0.0, to=1.0, orient="horizontal", variable=var_al, command=lambda v, idx=i: self._send_change(('layer', idx, 'alpha_live'), float(self.layer_vars[idx]['alpha_live'].get())))
            s_al.grid(row=1, column=1, sticky="we", padx=4, columnspan=1)

            ttk.Label(row, text="α_old:").grid(row=1, column=2, sticky="e")
            var_ao = tk.DoubleVar(value=0.5)
            s_ao = ttk.Scale(row, from_=0.0, to=1.0, orient="horizontal", variable=var_ao, command=lambda v, idx=i: self._send_change(('layer', idx, 'alpha_old'), float(self.layer_vars[idx]['alpha_old'].get())))
            s_ao.grid(row=1, column=3, sticky="we", padx=4, columnspan=1)

            # Rule & Palette
            ttk.Label(row, text="Rule:").grid(row=2, column=0, sticky="e")
            var_rule = tk.StringVar(value=self.RULE_LIST[0])
            cb_rule = ttk.Combobox(row, state="readonly", width=12, values=self.RULE_LIST, textvariable=var_rule)
            cb_rule.grid(row=2, column=1, sticky="w", padx=4)
            cb_rule.bind('<<ComboboxSelected>>', lambda e, idx=i: self._send_change(('layer', idx, 'rule'), self.layer_vars[idx]['rule'].get()))

            ttk.Label(row, text="Palette:").grid(row=2, column=2, sticky="e")
            var_pal = tk.StringVar(value=self.PALETTE_LIST[0])
            cb_pal = ttk.Combobox(row, state="readonly", width=12, values=self.PALETTE_LIST, textvariable=var_pal)
            cb_pal.grid(row=2, column=3, sticky="w", padx=4)
            cb_pal.bind('<<ComboboxSelected>>', lambda e, idx=i: self._send_change(('layer', idx, 'palette'), self.layer_vars[idx]['palette'].get()))
    ...
    def _update_from_app(self):
        try:
            if hasattr(self.app, 'tick_ms'): self.vars['tick_ms'].set(str(self.app.tick_ms))
            if hasattr(self.app, 'rms_strength'): self.vars['rms_strength'].set(str(self.app.rms_strength))
            if hasattr(self.app, 'max_age'): self.vars['max_age'].set(str(self.app.max_age))
            if hasattr(self.app, 'aging_speed'): self.vars['aging_speed'].set(str(self.app.aging_speed))
            if hasattr(self.app, 'layers'):
                lc = len(self.app.layers)
                self.vars['layer_count'].set(str(lc))
                self._rebuild_layers_ui(initial=True)
                # заполнить значения на слой
                for i, layer in enumerate(self.app.layers):
                    if i >= len(self.layer_vars): break
                    d = self.layer_vars[i]
                    for name in ('solo','mute','mix','alpha_live','alpha_old'):
                        if hasattr(layer, name):
                            try:
                                d[name].set(getattr(layer, name))
                            except Exception:
                                pass
        except Exception:
            pass
    ...
    def _update_loop(self):
        try:
            if hasattr(self.app, 'layers') and self.app.layers:
                import numpy as _np
                status_lines = []
                status_lines.append(f"Активных слоёв: {len([l for l in self.app.layers if not getattr(l, 'mute', False)])}")
                try:
                    total_cells = sum(_np.sum(getattr(l, 'grid', _np.array([]))) for l in self.app.layers)
                    status_lines.append(f"Всего клеток: {int(total_cells)}")
                except Exception:
                    pass
        ...
        if self.is_running and self.window:
            self.window.after(300, self._update_loop)
    ...
    def get_pending_changes(self):
        changes = []
        if self.settings_queue is None:
            return changes
        try:
            while True:
...
# === End Embedded SettingsWindow ===

# SimpleColors теперь определен в этом файле - импорт guitar_life больше не нужен
...
class HUD:
    """Простая заглушка HUD - минимальный интерфейс"""
    ...
    def handle_event(self, event):
        """Заглушка обработки событий"""
        return False
    
    def update_from_app(self, app):
        """Заглушка обновления из приложения"""
        pass
    
    def update_callbacks(self):
        """Заглушка обновления колбэков"""
        pass
    
    def draw(self, surface, info=None):
        """Минимальная отрисовка HUD"""
        if not self.visible:
            return
        ...
        panel_height = 100  # Небольшая панель
        ...
        # Заголовок
        title_text = "GUITAR-LIFE v13 (Minimal HUD)"
        title_surface = self.font.render(title_text, True, SimpleColors.PRIMARY)
        surface.blit(title_surface, (panel_x + 10, panel_y + 10))
        
        # Минимальная информация
        if info:
            info_text = f"FPS: {info.get('fps', 0):.1f} | Layers: {info.get('layer_count', 0)} | Cells: {info.get('total_cells', 0)}"
...
def get_palette_by_category(category: str = "all"):
    ...
    if category == "HSV-дизайны" or category == "HSV Палитры":
        return PALETTE_NAMES
...
def _cached_palette_hsv(palette_name: str, t: float) -> tuple[float, float, float]:
    ...
    if palette_name == "FIRE":
        return hue_fire_from_t(t)
    elif palette_name == "OCEAN":
        return hue_ocean_from_t(t)
    elif palette_name == "NEON":
        return hue_neon_from_t(t)
    elif palette_name == "UKRAINE":
        return hue_ukraine_from_t(t)
    elif palette_name == "RAINBOWSMOOTH":
        return hue_rainbow_smooth_from_t(t)
    elif palette_name == "SUNSET":
        return hue_sunset_from_t(t)
    elif palette_name == "AURORA":
        return hue_aurora_from_t(t)
    elif palette_name == "GALAXY":
        return hue_galaxy_from_t(t)
    elif palette_name == "TROPICAL":
        return hue_tropical_from_t(t)
    elif palette_name == "VOLCANO":
        return hue_volcano_from_t(t)
    elif palette_name == "DEEPSEA":
        return hue_deepsea_from_t(t)
    elif palette_name == "CYBERPUNK":
        return hue_cyberpunk_from_t(t)
    elif palette_name == "BGYR":
...
def blend_colors(color1: Tuple[int, int, int], color2: Tuple[int, int, int], 
    ...
    if blend_mode == "normal":
        ...
        def overlay_channel(base, overlay, factor):
            overlay = overlay * factor
            if base < 128:
                return int(2 * base * overlay / 255)
            else:
                return int(255 - 2 * (255 - base) * (255 - overlay) / 255)
...
def color_from_age_rms(age: int, rms: float, rms_strength: float,
                       ...
                       rms_enabled: bool = True) -> Tuple[int,int,int]:
    """
    ...
    if rms_mode == "palette":
        ...
        final_color = blend_colors(age_color, rms_color, mix_factor, blend_mode)
...
def color_from_age_only(age: int, fade_start: int, max_age: int,
    ...
    """Генерирует цвет только на основе возраста клетки."""
    a = min(age, max_age if max_age > 0 else age)
    t_age_raw = age_to_t(a, max_age if max_age > 0 else max(12, a+1))
    t_age = maybe_invert_t(t_age_raw)
    ...
    try:
        ...
        if result:
            ...
            if 'layers_cfg' in result and result['layers_cfg']:
                ...
                age_palettes = HSV_DESIGN_PALETTES  # Для возраста используем HSV-дизайны
                rms_palettes = HSV_COLOR_PALETTES   # Для RMS используем HSV палитры
                
                if diff_per_layer:
                    for i in range(layer_count):
                        layers_cfg.append({
                            'rule': rules_all[i % len(rules_all)],
                            'color_mode': result.get('color_mode', 'HSV-дизайны'),
            ...
            return {
                ...
                'aging_speed': result.get('aging_speed', 2.0),
...
class LayerConfig:
    ...
    initial_cells: int = 20  # Начальное количество клеток
...
class LayerGenerator:
    ...
    def create_layer_from_config(self, config: LayerConfig) -> Layer:
        ...
        spawn_cells(grid, config.initial_cells, config.spawn_method)
    ...
    def create_random_layer_config(self) -> LayerConfig:
        ...
        return LayerConfig(
            ...
            initial_cells=random.randint(10, 50)
        )
    ...
    def create_preset_configs(self, count: int, preset_type: str = "balanced") -> List[LayerConfig]:
        ...
        for i in range(count):
            ...
            config = LayerConfig(
                ...
                initial_cells=base_config.initial_cells + i * 5
            )
    ...
    def update_layer_transparency(self, layer: Layer, alpha_live: int, alpha_old: int):
        ...
        print(f"🔧 Прозрачность слоя обновлена: α_live={layer.alpha_live}, α_old={layer.alpha_old}")
...
class UIComboBox:
    ...
    def handle_event(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            if self.expanded:
                drop = self._dropdown_rect()
                if not drop.collidepoint(event.pos):
                    self.expanded = False
                    return False

        # wheel scroll
        if self.expanded and event.type == pygame.MOUSEWHEEL and len(self.options) > self.max_visible:
            self.scroll = max(0, min(self.scroll - event.y, len(self.options) - self.max_visible))
            return True

        # select with click inside dropdown
        if self.expanded and event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            idx = self._item_index_at(event.pos)
            if idx is not None:
                old = self.current_index
                self.current_index = idx
                self.expanded = False
                if self.callback and self.param_name and old != idx:
                    self.callback(self.param_name, self.options[idx])
                return True

        # keyboard navigation
        if self.expanded and event.type == pygame.KEYDOWN:
            if event.key in (pygame.K_UP, pygame.K_w):
                self.selected_index = max(0, self.current_index - 1)
                return True
            if event.key in (pygame.K_DOWN, pygame.K_s):
                self.selected_index = min(len(self.options) - 1, self.current_index + 1)
                return True
    ...
    def draw(self, surface):
        # main button
        pygame.draw.rect(surface, self.colors.get('button_bg', (20,20,20)), self.rect)
        pygame.draw.rect(surface, self.colors.get('border', (80,80,80)), self.rect, 1)

        current_text = f"{self.label}: {self.get_selected()}" if self.label else f"{self.get_selected()}"
        text_surface = self.font.render(current_text, True, self.colors.get('text', (220,220,220)))
        text_y = self.rect.y + (self.rect.height - text_surface.get_height()) // 2
        surface.blit(text_surface, (self.rect.x + 6, text_y))

        arrow = "▲" if self.expanded else "▼"
        arrow_surface = self.font.render(arrow, True, self.colors.get('text', (220,220,220)))
        surface.blit(arrow_surface, (self.rect.right - arrow_surface.get_width() - 6, text_y))

        # dropdown
        if self.expanded and self.options:
            drop = self._dropdown_rect()
            pygame.draw.rect(surface, self.colors.get('button_bg', (20,20,20)), drop)
            pygame.draw.rect(surface, self.colors.get('border', (80,80,80)), drop, 1)

            start = self.scroll
            end = min(start + self.max_visible, len(self.options))
            for i, option in enumerate(self.options[start:end], start):
                option_rect = pygame.Rect(drop.x, drop.y + (i - start) * self.item_h, drop.width, self.item_h)
                hovered = option_rect.collidepoint(pygame.mouse.get_pos())
                bg = self.colors.get('button_hover', (60,60,60)) if hovered or (i == self.current_index) else self.colors.get('button_bg', (20,20,20))
                pygame.draw.rect(surface, bg, option_rect)
                pygame.draw.rect(surface, self.colors.get('border', (80,80,80)), option_rect, 1)
                opt_surface = self.font.render(str(option), True, self.colors.get('text', (220,220,220)))
                surface.blit(opt_surface, (option_rect.x + 6, option_rect.y + (option_rect.height - opt_surface.get_height()) // 2))
...
def build_color_image(layer_grid: np.ndarray, layer_age: np.ndarray, mode: str,
                      ...
                      rms_mode: str = "brightness",
                      blend_mode: str = "normal",
                      rms_enabled: bool = True) -> np.ndarray:
    ...
    # Debug отключен для производительности
    # if len(ys) > 0:
    #     max_y, max_x = np.max(ys), np.max(xs)
    #     min_y, min_x = np.min(ys), np.min(xs)
    #     if max_y >= H or max_x >= W or min_y < 0 or min_x < 0:
    #         print(f"🚨 NONZERO ERROR: np.nonzero returned invalid indices")
    #         print(f"   Grid shape: {H}x{W}, nonzero range: y=[{min_y},{max_y}], x=[{min_x},{max_x}]")
    #         # Filter out invalid indices
    #         valid_mask = (ys >= 0) & (ys < H) & (xs >= 0) & (xs < W)
    #         ys, xs = ys[valid_mask], xs[valid_mask]
    #         print(f"   Filtered to {len(ys)} valid indices")
    
    for idx, (i, j) in enumerate(zip(ys, xs)):
        # Преобразуем индексы в int для безопасности
        i, j = int(i), int(j)
        
        # CRITICAL: Add bounds checking to prevent index errors
        if i < 0 or i >= H or j < 0 or j >= W:
            # print(f"🚨 BOUNDS ERROR: Attempting to access grid[{i},{j}] but grid is only {H}x{W}")
            continue
            
        # Additional safety check for age array
        if i >= layer_age.shape[0] or j >= layer_age.shape[1]:
            # print(f"🚨 AGE BOUNDS ERROR: Attempting to access age[{i},{j}] but age array is {layer_age.shape}")
            continue
        ...
        try:
            if mode == "HSV Палитры":
                ...
                color = color_from_age_rms(int(layer_age[i, j]), rms, rms_strength,
                                           ...
                                           rms_mode, blend_mode, rms_enabled)
            
            # Устанавливаем RGB цвет
            img[i, j] = color
        except Exception as e:
            if idx < 5:  # Выводим только первые 5 ошибок чтобы не спамить
                print(f"COLOR ERROR at cell ({i},{j}), age={layer_age[i,j] if i < layer_age.shape[0] and j < layer_age.shape[1] else 'out_of_bounds'}: {e}")
                print(f"  Mode: {mode}, Age palette: {age_palette}, RMS palette: {rms_palette}")
                print(f"  RMS mode: {rms_mode}, blend: {blend_mode}, rms_enabled: {rms_enabled}")
            # Используем белый цвет по умолчанию при ошибке
            img[i, j] = (255, 255, 255)
...
class App:
    def __init__(self, sel: Dict[str, Any]):
        ...
        pygame.display.set_caption("Guitar Life v13 — Ultimate [F12 = Settings, H = HUD]")
        ...
        if SETTINGS_WINDOW_AVAILABLE:
            ...
            print("✅ Окно настроек инициализировано")
            ...
            if use_config_file:
                ...
                if i < len(layers_cfg):
                    ...
                    layer_params = {
                        ...
                        'color_mode': row.get('color_mode', 'HSV-дизайны'),
                    ...
                    print(f"WARNING: layers_cfg[{i}] not found, using defaults")
                    layer_params = {
                        'rule': 'Conway',
                        'age_palette': 'Fire',
                        'rms_palette': 'Ocean',
        ...
        self.aging_speed = sel.get('aging_speed', 1.0)  # Ускорение старения (умножитель)
        ...
        self.max_cells_percent = sel.get('max_cells_percent', 80)  # Максимальный процент заполнения сетки
        ...
        self.old_cells_priority = sel.get('old_cells_priority', True)  # Приоритет старых клеток при удалении
        self.clear_type = sel.get('clear_type', 'Полная очистка')  # Тип очистки для кнопки Clear
        self.clear_partial_percent = sel.get('clear_partial_percent', 50)  # Процент для частичной очистки
        self.clear_age_threshold = sel.get('clear_age_threshold', 10)  # Возрастной порог для очистки по возрасту
    ...
    def apply_joy_division(self):
        ...
        for L in self.layers:
            ...
            L.rms_palette = "White->LightGray->DarkGray"
    ...
    def on_hud_parameter_change(self, param_name: str, value):
        ...
        # Палитры для отдельных слоев
            ...
            if layer_idx < len(self.layers):
                ...
                print(f"🎛️ Layer {layer_idx+1} RMS Mode changed to: {rms_mode}")
                self.save_layer_settings()  # Сохраняем настройки
                self.hud.update_from_app(self)  # Обновляем GUI
            ...
            if layer_idx < len(self.layers):
                ...
                self.save_layer_settings()  # Сохраняем настройки
                self.hud.update_from_app(self)  # Обновляем GUI
        elif param_name.startswith('layer_') and '_rms_enabled' in param_name:
            # Извлекаем номер слоя из имени параметра
            layer_idx = int(param_name.split('_')[1])
            if layer_idx < len(self.layers):
                self.layers[layer_idx].rms_enabled = bool(value)
                print(f"� Layer {layer_idx+1} RMS Enabled changed to: {value}")
                self.save_layer_settings()  # Сохраняем настройки
    ...
    def cycle_layer_rule(self, layer_idx: int):
        ...
        if layer_idx < 0 or layer_idx >= len(self.layers):
            print(f"⚠️ Invalid layer index: {layer_idx}")
            return
    ...
    def soft_clear(self):
        ...
        if self.soft_mode == "Удалять клетки":
            ...
            for L in self.layers:
                ...
                if k > 0:
                    if self.old_cells_priority:
                        # Удаляем старые клетки в приоритете
                        alive_positions = np.where(g)
                        ages = L.age[alive_positions]
                        sorted_indices = np.argsort(ages)[::-1]  # От старых к молодым
            ...
            # Только мягкое затухание яркости
            ...
            fade_kill_rate = self.soft_kill_rate * 0.5  # Уменьшаем интенсивность удаления в комбо режиме
            for L in self.layers:
                g = L.grid
                if not g.any(): continue
                alive = np.argwhere(g)
    ...
    def soft_population_control(self):
        ...
        # Векторизованная обработка всех слоев
        for i, L in enumerate(self.layers):
            ...
            # Вычисляем интенсивность удаления векторизованно
            if total_cells > max_allowed_cells:
                excess_ratio = (total_cells - max_allowed_cells) / max_allowed_cells
                base_removal = int(total_cells * self.soft_kill_rate / 100.0)
            ...
            # Векторизованное получение позиций живых клеток
            alive_mask = L.grid
            if not np.any(alive_mask):
                continue
                
            alive_r, alive_c = np.where(alive_mask)
            ages = L.age[alive_r, alive_c]
            
            # Векторизованная сортировка/выбор индексов для удаления
            if self.old_cells_priority and self.age_bias > np.random.randint(0, 100):
                # Удаляем по возрасту (векторизованная сортировка)
                sorted_indices = np.argsort(-ages)  # Отрицательный знак для сортировки по убыванию
            else:
                # Случайный выбор (векторизованный)
            ...
            # Векторизованное применение различных режимов удаления
            if self.soft_mode == "Удалять клетки":
                ...
                debug_info[-1] += f" → removed {half_point} + aged {num_to_remove - half_point} cells"
        
        # Векторизованный вывод отладочной информации
        if debug_info:
            self.debug_counter = getattr(self, 'debug_counter', 0) + 1
            if self.debug_counter % 60 == 0:  # Каждую секунду при 60 FPS
    ...
    def update_layers(self, births: int):
        ...
        # Векторизованное распределение рождений по слоям
        if births > 0 and self.layers:
            per = max(1, births // len(self.layers))
            # Параллельная обработка спавна для всех слоев
            for L in self.layers:
                spawn_cells(L.grid, per, self.spawn_method)
        ...
        for i, L in enumerate(self.layers):
            try:
                ...
                alive_mask = L.grid
                L.age[alive_mask] += effective_increment
                L.age[~alive_mask] = 0
                ...
                if L.grid.shape != (GRID_H, GRID_W):
                    # print(f"🚨 CRITICAL: Layer {i} grid shape corrupted: {L.grid.shape} != ({GRID_H}, {GRID_W})")
                    # print(f"   Recreating grid with correct shape...")
                    new_grid = np.zeros((GRID_H, GRID_W), dtype=bool)
                    if L.grid.size > 0:
                        copy_h = min(L.grid.shape[0], GRID_H)
                        copy_w = min(L.grid.shape[1], GRID_W)
                        new_grid[:copy_h, :copy_w] = L.grid[:copy_h, :copy_w]
                    L.grid = new_grid
                
                # SAFETY: Векторизованная проверка формы массива возраста
                if L.age.shape != L.grid.shape:
                ...
                L.grid[[0, -1], :] = False  # Верхний и нижний края одновременно
                L.grid[:, [0, -1]] = False  # Левый и правый края одновременно
                ...
                if self.mirror_x:
                    L.grid = np.fliplr(L.grid)
                if self.mirror_y:
                    L.grid = np.flipud(L.grid)
                    
            except Exception as e:
                print(f"🚨 ERROR updating layer {i}: {e}")
                print(f"   Layer rule: {getattr(L, 'rule', 'Unknown')}")
                print(f"   Grid shape: {getattr(L.grid, 'shape', 'Unknown') if hasattr(L, 'grid') else 'No grid'}")
                
                # Emergency fallback - recreate layer
                try:
                    L.grid = np.zeros((GRID_H, GRID_W), dtype=bool)
                    L.age = np.zeros((GRID_H, GRID_W), dtype=np.int32)
    ...
    def render(self, rms: float, pitch: float):
        ...
        for i, L in enumerate(layers):
            # live_cells = np.sum(L.grid)  # Убираем подсчет для каждого кадра
            ...
            try:
                ...
                print(f"  Layer info: rule={L.rule}, color_mode={L.color_mode}")
                print(f"  Palettes: age={L.age_palette}, rms={L.rms_palette}")
                print(f"  RMS mode: {L.rms_mode}, blend: {L.blend_mode}, rms_enabled: {L.rms_enabled}")
                print(f"  Grid shape: {L.grid.shape}, Age shape: {L.age.shape}")
                print(f"  RMS: {rms}, Pitch: {pitch}")
                import traceback
                traceback.print_exc()
                # Пропускаем этот слой и продолжаем
                continue
    ...
    def save_layer_settings(self):
        ...
        try:
            ...
            if resource_manager:
                ...
                print("✅ Layer settings saved to app_config.json")
    ...
    def run(self):          
        ...
        # Профилирование для диагностики производительности
        import time
        profile_data = {}
        ...
        while running:
            ...
            for ev in pygame.event.get():
                ...
                if ev.type == pygame.QUIT:
                    ...
                    if ev.key == pygame.K_ESCAPE:
                        ...
                        print(f"🔧 DEBUG: Ctrl+S нажато! SETTINGS_WINDOW_AVAILABLE={SETTINGS_WINDOW_AVAILABLE}")
                        print(f"🔧 DEBUG: self.settings_window={self.settings_window}")
                        
                        if self.settings_window and SETTINGS_WINDOW_AVAILABLE:
                            print(f"🔧 DEBUG: is_running={self.settings_window.is_running}")
                            if not self.settings_window.is_running:
                                try:
                                    self.settings_window.start()
                                    print("🔧 Окно настроек открыто (Ctrl+S)")
                                except Exception as e:
                                    print(f"❌ Ошибка открытия окна настроек: {e}")
                            else:
                                print("⚠️  Окно настроек уже открыто")
                        else:
                            print("⚠️  Окно настроек недоступно")
                            if not SETTINGS_WINDOW_AVAILABLE:
                                print("   - SETTINGS_WINDOW_AVAILABLE = False")
                            if not self.settings_window:
                                print("   - self.settings_window = None")
                    elif ev.key == pygame.K_F12:
                        # F12 - альтернативная горячая клавиша для окна настроек
                        print("🔧 DEBUG: F12 нажато!")
                        if self.settings_window and SETTINGS_WINDOW_AVAILABLE:
                            if not self.settings_window.is_running:
                                try:
                                    self.settings_window.start()
                                    print("🔧 Окно настроек открыто (F12)")
                                except Exception as e:
                                    print(f"❌ Ошибка открытия окна настроек: {e}")
                            else:
                                print("⚠️  Окно настроек уже открыто")
            ...
            if self.settings_window and SETTINGS_WINDOW_AVAILABLE:
                ...
                for param_name, value in changes:
                    try:
                        ...
                        print(f"🔧 Применено из окна настроек: {param_name} = {value}")
                    except Exception as e:
                        print(f"⚠️  Ошибка применения настройки {param_name}: {e}")
            ...
            if self.hud.visible and not use_simple_hud:
                ...
                if not hasattr(self, '_hud_last_update'):
                    ...
                    self._hud_update_interval = 1.0 / 15.0  # Очень низкая частота для HUD
...
def blit_layer(self, img, mix, alpha_live, alpha_old):
    """Composite a layer surface 'img' onto self.target using pygame special_flags.
    mix: 'normal'|'add'|'multiply'|'max'|'min'
    alpha_live/alpha_old are expected to be already baked into img; if not, set img.set_alpha().
    """
    import pygame
    if img is None: 
        return
    # Ensure alpha set if provided (0..1)
    try:
        if isinstance(alpha_live, (int, float)):
            a = max(0, min(1.0, float(alpha_live)))
            img.set_alpha(int(a * 255))
    except Exception:
        pass
    ...
    if not isinstance(mix, str):
        mix = 'normal'
    m = mix.lower()
    if m in ('add', 'additive'):
        flags = pygame.BLEND_RGB_ADD
    elif m in ('multiply', 'mul'):
        flags = pygame.BLEND_MULT
    elif m == 'max':
        flags = pygame.BLEND_RGBA_MAX
    elif m == 'min':
        flags = pygame.BLEND_RGBA_MIN
    else:
        flags = 0  # normal

    # blit to target surface
    try:
        self.target.blit(img, (0,0), special_flags=flags)
    except TypeError:
        # fallback for older pygame without special_flags param
        self.target.blit(img, (0,0))
