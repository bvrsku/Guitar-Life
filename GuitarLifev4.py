...
HSV_COLOR_PALETTES = HSV_DESIGN_PALETTES.copy()
PALETTE_OPTIONS = HSV_DESIGN_PALETTES + HSV_COLOR_PALETTES
ALL_PALETTES = PALETTE_OPTIONS
...
class App:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.last_auto_clear_time = time.time()
    ...
    def _handle_keydown(self, event: pygame.event.Event):
        ...
        if key == pygame.K_ESCAPE:
            ...
            self.auto_clear_enabled = not self.auto_clear_enabled
            print(f"🔄 Автоочистка {'включена' if self.auto_clear_enabled else 'отключена'}")
    ...
    def update_simulation(self):
        ...
        # Автоматическая очистка при низком RMS
    ...
    def _gather_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'Auto Clear': 'ON' if self.auto_clear_enabled else 'OFF',
            'Clear Threshold': f"{DEFAULT_CLEAR_RMS:.3f}",
    ...
    def _check_auto_clear(self):
        """Проверка необходимости автоматической очистки на основе RMS"""
        if not self.auto_clear_enabled:
            return
        
        current_time = time.time()
        # Проверяем каждые 2 секунды
        if current_time - self.last_auto_clear_time < 2.0:
            return
        
        # Проверяем RMS уровень
        if audio_rms < DEFAULT_CLEAR_RMS:
            # Подсчитываем общее количество живых клеток
            total_cells = sum(np.sum(layer.grid) for layer in self.layers if not layer.mute)
            
            # Если есть живые клетки и RMS низкий, очищаем
            if total_cells > 0:
                self._clear_all_layers()
                print(f"🧹 Автоочистка: RMS={audio_rms:.4f} < {DEFAULT_CLEAR_RMS}")
        
        self.last_auto_clear_time = current_time
    ...
    def _show_help(self):
        ...
        A - Включить/выключить автоочистку (RMS < {})
        F1 - Эта справка
        F12 - Окно настроек
        1-9 - Solo слоя
        
        Мышь - Рисование в области сетки
        """.format(DEFAULT_CLEAR_RMS)
        print(help_text)
