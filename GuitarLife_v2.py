...
class HUD:
    ...
    def _create_layer_modules(self):
        ...
        module_height = 80  # Высота каждого модуля
    ...
    def update_from_app(self, app):
        ...
        for i, module in enumerate(self.layer_modules):
            ...
            if 'aging_speed' in controls:
                controls['aging_speed'].current_val = getattr(layer, 'aging_speed', 1.0)
        ...
        for control_name, control in controls.items():
            if hasattr(control, 'draw'):
                ...
                if (control.y + control.height >= self.panel_y + 60 and 
                    ...
                    # Отрисовка слайдера: label уже рисуется внутри UISlider.draw, не дублируем
...
def build_color_image(layer_grid: np.ndarray, layer_age: np.ndarray, mode: str,
    ...
    for idx, (i, j) in enumerate(zip(ys, xs)):
        ...
        try:
            # Новый режим: только два варианта
            if mode == "Только RMS":
...
class App:
    ...
    def on_hud_parameter_change(self, param_name: str, value):
        ...
        if param_name.startswith('layer_') and param_name.endswith('_aging_speed'):
            ...
            if layer_idx < len(self.layers):
                aging_speed = max(0.1, min(10.0, float(value)))
    ...
    def update_layers(self, births: int):
        ...
        for i, L in enumerate(self.layers):
            try:
                ...
                # SAFETY: Ensure age array matches grid shape and dtype
            if L.age.shape != L.grid.shape or L.age.dtype != np.int32:
                # Безопасное увеличение возраста с защитой от переполнения
                # Ограничиваем возраст клеток до L.max_age для предотвращения переполнения
                L.age[alive_mask] = np.clip(
                    L.age[alive_mask] + effective_increment, 0,
                    getattr(L, 'max_age', np.iinfo(L.age.dtype).max)
                )
                L.age[~alive_mask] = (
                    0,
                    np.iinfo(L.age.dtype).max
                )
    ...
    def render(self, rms: float, pitch: float):
        ...
        for i, L in enumerate(layers):
            ...
            try:
                img = build_color_image(L.grid, L.age, "Возраст + RMS", rms, pitch, cfg,
                ...
                self.renderer.last_age_mask = L.age.copy()
                self.renderer.last_grid_mask = L.grid.copy()
                self.renderer.last_max_age = L.max_age
...
#class HUD:
    ...
    def _draw_header(self, surface, base_x, y, info):
        title = self.font.render("AUTOMATA LAYER CONTROLS", True, HUD_HEADER)    #
