...
def _create_optimized_age_array(height: int, width: int) -> np.ndarray:
    """Создает оптимизированный массив возраста клеток"""
...
def step_life(grid: np.ndarray, rule: str, out: Optional[np.ndarray] = None) -> np.ndarray:
    """Выполняет один шаг клеточного автомата с возможностью in-place обновления"""
    H, W = grid.shape
...
def clear_grid(grid: np.ndarray, clear_type: str = "full", partial_ratio: float = 0.5) -> None:
    ...
    if clear_type == "full":
        ...
        grid[:, 0] = False
        grid[:, -1] = False
    elif clear_type == "age_based":
        # Очищаем старые клетки
        grid[grid.shape[0] // 2, grid.shape[1] // 2] = False
    elif clear_type == "random":
        # Очищаем случайные клетки
        grid[np.random.rand(*grid.shape) < partial_ratio] = False
    elif clear_type == "custom":
        # Очищаем клетки по заданному условию
...
def choose_settings() -> Optional[Dict[str, Any]]:
    ...
    if not SD_AVAILABLE:
        ...
        return {"device": None, "layer_count": 1}
    ...
    try:
        ...
        return {
            ...
            "layer_count": 1,  # Используем 1 слой как в референсной версии
            "mirror_x": False,
            "mirror_y": False
        }
    except Exception as e:
...
class LayerGenerator:
    ...
    def create_preset_layer(preset_name: str, grid_shape: Tuple[int, int]) -> Layer:
        ...
        presets = {
            "fire": Layer(grid=grid, age=age, rule="Conway", age_palette="Fire", rms_palette="Sunset"),
            "ocean": Layer(grid=grid, age=age, rule="HighLife", age_palette="Ocean", rms_palette="DeepSea"),
            "neon": Layer(grid=grid, age=age, rule="Day&Night", age_palette="Neon", rms_palette="Cyberpunk"),
            "nature": Layer(grid=grid, age=age, rule="Coral", age_palette="Forest", rms_palette="Spring")
        }
        
        return presets.get(preset_name, presets["fire"])
...
class RenderManager:
    ...
    def render_layer(self, layer: Layer, surface: pygame.Surface, 
        ...
        img = self.build_color_image(layer.grid, layer.age, rms, config, layer)

        # Конвертируем в pygame surface
        try:
            layer_surface = pygame.surfarray.make_surface(np.transpose(img, (1, 0, 2)))
    ...
    def build_color_image(self, layer_grid: np.ndarray, layer_age: np.ndarray, 
                         rms: float, config: Dict[str, Any], layer: Layer) -> np.ndarray:
        """
        Создание цветного изображения слоя.

        Args:
            layer_grid (np.ndarray): Массив сетки клеток.
            layer_age (np.ndarray): Массив возрастов клеток.
...
class HUD:
    ...
    def __init__(self, font: pygame.font.Font, screen_height: int, layer_count: int = 1):
        self.font = font
        self.visible = True
...
class App:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Конфигурация
        self.config = config
        self.layer_count = config.get('layer_count', 1)
    ...
    def render(self):
        ...
        # Создаем поверхность для слоев
        layer_surface = pygame.Surface((self.W, self.H))
        ...
        self.screen.blit(layer_surface, (HUD_WIDTH, 0))
        # Рендерим HUD
        if self.hud.visible:
...
def main():
    ...
    try:
        ...
        try:
            ...
            # Создаем тестовый паттерн для проверки работы
